            ...
            <form id="form-m7-qa-approve">
                <div style="display:flex; gap:12px; margin-bottom:15px; flex-wrap:wrap;">
                    <div style="background: rgba(255,255,255,0.05); padding: 8px 12px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1); flex: 1;">
                        <label style="font-size: 0.75em; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; display:block; margin-bottom:4px;">Appraisal Tool</label>
                        <input type="text" id="m7-qa-tool" value="${q.tool}" style="width:100%; font-size: 1.1em; font-weight: bold; background: #222; color: #fff; border: 1px solid #444; padding: 6px; border-radius: 4px;" required>
                    </div>
                    <div style="background: rgba(255,255,255,0.05); padding: 8px 12px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1);">
                        <label style="font-size: 0.75em; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; display:block; margin-bottom:4px;">Batas Lulus (%)</label>
                        <input type="number" id="m7-qa-threshold" value="${q.threshold}" style="width:80px; font-size: 1.1em; font-weight: bold; background: #222; color: #38bdf8; border: 1px solid #444; padding: 6px; border-radius: 4px;" required>
                    </div>
                </div>

                <div style="background: rgba(255,255,255,0.05); padding: 8px 12px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px;">
                    <label style="font-size: 0.75em; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; display:block; margin-bottom:4px;">Kategorisasi Output</label>
                    <input type="text" id="m7-qa-categorization" value="${q.categorization || ''}" style="width:100%; font-size: 0.9em; background: #222; color: #cbd5e1; border: 1px solid #444; padding: 6px; border-radius: 4px;">
                </div>

                <div style="background: rgba(0,0,0,0.15); padding: 12px; border-left: 3px solid #38bdf8; border-radius: 4px; margin-bottom: 15px;">
                    <label style="font-size: 0.75em; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; display:block; margin-bottom:4px;">Justifikasi & Rubrik Penilaian</label>
                    <textarea id="m7-qa-justification" style="width:100%; height: 180px; font-size: 0.88em; color:#cbd5e1; line-height: 1.6; background: #222; border: 1px solid #444; padding: 8px; border-radius: 4px; resize: vertical;" required>${q.tool_justification || ''}</textarea>
                </div>
                ...
                <div style="margin-top: 15px;">
                    <button type="submit" class="btn btn-success" style="width: 100%; font-weight: bold;">Setujui & Lanjutkan QA</button>
                </div>
            </form>
        `);
        
        setTimeout(() => {
            const form = document.getElementById('form-m7-qa-approve');
            if (form) {
                form.addEventListener('submit', async (e) => {
                    ...
                    const payload = {
                        qa_threshold: {
                            tool: document.getElementById('m7-qa-tool').value,
                            threshold: parseFloat(document.getElementById('m7-qa-threshold').value),
                            categorization: document.getElementById('m7-qa-categorization').value,
                            justification: document.getElementById('m7-qa-justification').value
                        }
                    };
